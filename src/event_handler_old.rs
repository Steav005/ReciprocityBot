...
type EventHandlerConfigType =
    Arc<RwLock<HashMap<ChannelId, (Sender<Event>, Arc<RwLock<HashSet<MessageId>>>)>>>;
...
impl ReciprocityEventHandler {
    ...
    async fn handle_event(&self, handler: Sender<Event>, channel: ChannelId, event: Event) {
        ...
        if let Err(err) = handler.send(event).await {
            warn!(
                "Receiver for {:?}, were dropped. Removing Guild. {:?}",
                channel, err
            );
    ...
    pub async fn process_event_new_message(&self, msg: Message) {
        ...
        self.handle_event(handler, msg.channel_id, Event::NewMessage(msg.id))
            .await;
    }
    ...
    pub async fn process_event_message_deleted(&self, channel: ChannelId, msg: MessageId) {
        ...
        self.handle_event(handler, channel, Event::DeletedMessage(msg))
            .await;
    }
    ...
    pub async fn add_guild(&self, channel: ChannelId) -> Result<Receiver<Event>, Error> {
    ...
    async fn get_channel(
        &self,
        channel: ChannelId,
    ) -> Result<Arc<RwLock<HashSet<MessageId, RandomState>>>, Error> {
